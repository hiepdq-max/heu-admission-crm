-- Step 33 correction - HEU actual training catalog
-- Run after step33_program_major_master.sql if old sample programs were inserted.

insert into public.admission_programs (program_code, program_name, sort_order, status) values
  ('TRUNG_CAP', 'Trung cấp', 10, 'ACTIVE'),
  ('NGAN_HAN', 'Ngắn hạn', 20, 'ACTIVE'),
  ('LIEN_THONG_DAI_HOC', 'Liên thông đại học', 30, 'ACTIVE')
...
insert into public.admission_majors (major_code, major_name, program_id, sort_order, status)
select v.major_code, v.major_name, p.id, v.sort_order, 'ACTIVE'::public.record_status
from (
  values
...
set applies_to_program = null, updated_at = now()
where applies_to_program in ('9+', 'HE_9_PLUS', 'CAO_DANG', 'TTGDTX');
