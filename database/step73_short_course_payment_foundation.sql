...
select
  ...
  (select count(*)::int from public.short_attendance_records where record_status = 'ACTIVE') as attendance_record_count,
  ...
  (select count(*)::int from public.short_governance_rules where status = 'ACTIVE'::public.record_status) as governance_rule_count,
  (select count(*)::int from public.short_risk_alerts where record_status = 'ACTIVE' and alert_status in ('OPEN', 'IN_REVIEW')) as open_risk_alert_count,
  (select count(*)::int from public.admission_offering_catalog where status = 'ACTIVE'::public.record_status) as offering_count,
  (select count(*)::int from public.short_student_profile_readiness where readiness_status = 'READY_TO_VERIFY') as student_ready_count,
  (select count(*)::int from public.short_student_profile_readiness where readiness_status in ('BLOCKED', 'NEEDS_DATA', 'NEEDS_FIX')) as student_needs_fix_count,
  (select count(*)::int from public.short_class_opening_readiness where readiness_status = 'READY_TO_OPEN') as class_ready_to_open_count,
  (select count(*)::int from public.short_class_opening_readiness where readiness_status in ('BLOCKED', 'NEEDS_FIX', 'OVER_CAPACITY')) as class_needs_fix_count,
  (select count(*)::int from public.short_enrollment_class_readiness where readiness_status = 'READY_TO_VERIFY') as enrollment_ready_count,
  (select count(*)::int from public.short_enrollment_class_readiness where readiness_status in ('BLOCKED', 'NEEDS_CLASS', 'NEEDS_FIX', 'CANCELLED')) as enrollment_needs_fix_count,
  ...
  (select count(*)::int from public.short_attendance_session_readiness where readiness_status in ('BLOCKED', 'NEEDS_ATTENDANCE', 'NEEDS_FIX', 'CANCELLED')) as attendance_needs_fix_count,
  (select count(*)::int from public.short_bhxh_policy_readiness where readiness_status = 'READY_TO_SUBMIT') as bhxh_ready_to_submit_count,
  (select count(*)::int from public.short_bhxh_policy_readiness where readiness_status in ('BLOCKED', 'NEEDS_ATTENDANCE', 'NEEDS_EVIDENCE', 'NEEDS_AMOUNT', 'NEEDS_FIX')) as bhxh_needs_fix_count,
