...
requireText(
  "docs/TTGDTX_SIGNED_UAT_EXECUTION_ROUTING_HUB_20260628.md",
  /UAT-ROUTE-11[\s\S]*P0-17 access closure decision[\s\S]*P0-17 access closure is missing/i,
...
requireText(
  "docs/HEU_IMPLEMENTATION_LOG.md",
  /UAT Routing Finance Day-1 Ledger Handoff[\s\S]*UAT-ROUTE-08[\s\S]*Finance Day-1 result ledger[\s\S]*UAT-ROUTE-11[\s\S]*final owner GO\/NO-GO[\s\S]*HEU_FINANCE_DAY1_RESULT_LEDGER_TEMPLATE_20260630\.md[\s\S]*audit-ttgdtx-signed-uat-execution-routing-hub\.mjs[\s\S]*does not execute UAT[\s\S]*mark production GO/i,
...
requireText(
  "components/reports/report-view-source-map-panel.tsx",
  /(?=[\s\S]*data-heu-report-view-evidence-attachment-queue="RV-EVID-01)(?=[\s\S]*Evidence attachment queue)(?=[\s\S]*RV-EVID-01)(?=[\s\S]*P5-03-TRIAL-EVID-001 through P5-03-TRIAL-EVID-005)(?=[\s\S]*FIN-START-EVID-001 through FIN-START-EVID-005)(?=[\s\S]*FIN-DAY1-EVID-001 through FIN-DAY1-EVID-005)(?=[\s\S]*P5_03_CONTROLLED_TRIAL_READY \/ FIN_START_READY \/ FIN_DAY1_RESULT_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*RV-EVID-06)(?=[\s\S]*AUDIT_AI_SCOPE_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*does not upload files,\s+accept evidence or waive blockers)/i,
...
requireText(
  "docs/HEU_IMPLEMENTATION_LOG.md",
  /Report View Finance Day-1 Evidence Gate[\s\S]*RV_TTGDTX_FINANCE_SUMMARY[\s\S]*Finance Day-1 start-gate and result\s+ledger evidence[\s\S]*FIN-START-EVID-001[\s\S]*FIN-DAY1-EVID-005[\s\S]*audit-heu-p0-register-pack\.mjs[\s\S]*audit-ttgdtx-release-gates\.mjs[\s\S]*does not upload files[\s\S]*approve report-view reliance[\s\S]*mark production GO/i,
...
requireText(
  "components/finance/finance-day-one-accountant-handoff.tsx",
  /(?=[\s\S]*data-finance-day-one-accountant-handoff="P5-03_FIN_DAY1_OPERATOR")(?=[\s\S]*Finance Day-1 accountant handoff: read-only pilot)(?=[\s\S]*FIN_ACCOUNTANT_HANDOFF_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*FIN-ACCT-HANDOFF-01[\s\S]*Allowed read-only review)(?=[\s\S]*FIN-ACCT-HANDOFF-04[\s\S]*Day-1 evidence closure)(?=[\s\S]*Data variance goes to KHTC owner)(?=[\s\S]*route\/scope issue goes to IT_DATA)(?=[\s\S]*legal\/source exception goes to PHAP_CHE)(?=[\s\S]*access leak goes to Audit)(?=[\s\S]*HEU_FINANCE_DAY1_ACCOUNTANT_OPERATOR_GUIDE_20260702\.md)(?=[\s\S]*start conditions)(?=[\s\S]*read-only steps)(?=[\s\S]*forbidden content)(?=[\s\S]*post vouchers, move money, issue\s+bank instructions or mark production GO)/i,
...
requireText(
  "app/finance-desk/page.tsx",
  /import \{ FinanceDayOneAccountantHandoff \}[\s\S]*import \{ FinanceDeskUatEvidenceChecklist \}[\s\S]*<FinanceDeskReadOnlyBoundary \/>[\s\S]*<FinanceOfficialOperationGate \/>[\s\S]*<FinanceDayOneAccountantHandoff \/>[\s\S]*<FinanceDeskRelianceDecisionManifest \/>[\s\S]*<FinanceDeskUatEvidenceChecklist \/>[\s\S]*!canOpen/i,
...
requireText(
  "docs/HEU_FINANCE_DESK_UAT_RUNBOOK_20260627.md",
  /(?=[\s\S]*Finance Desk Reliance Decision Manifest)(?=[\s\S]*Immediate stop guard)(?=[\s\S]*P5_03_RELIANCE_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*P5-03-REL-01[\s\S]*P5-03-REL-06)(?=[\s\S]*P5-03-ACCEPT-01 through P5-03-ACCEPT-06 and P5-03-REL-01 through\s+P5-03-REL-06)/i,
...
requireText(
  "docs/TTGDTX_9PLUS_PILOT_PRODUCTION_CHECKLIST.md",
  /No AI approval[\s\S]*PASS_LOCAL[\s\S]*HEU_CLOUD_AGENT_OPERATING_PLAN_20260702\.md[\s\S]*P7-06 is PASS_LOCAL_PLAN with CLOUD_AGENT_PLAN_READY \/ NO_GO \/ BLOCKED only[\s\S]*USD 20-40 monthly planning cap[\s\S]*owner setup checklist and stop conditions[\s\S]*cannot buy a server, enter payment details, create cloud infrastructure, store secrets, create users, send real email, create real tasks, accept UAT\/evidence, approve finance\/owner decisions or mark production GO/i,
...
requireText(
  "docs/HEU_ROLE_SCOPE_UAT_EXECUTION_PACK_20260627.md",
  /(?=[\s\S]*Real Accounting User UAT Queue)(?=[\s\S]*data-heu-real-accounting-user-uat-queue="P6-04-P2-18-P5-03")(?=[\s\S]*REAL-ACC-01)(?=[\s\S]*REAL-ACC-06)(?=[\s\S]*REAL_USER_SCOPE_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*KHTC accounting operator)(?=[\s\S]*BGH read-only reviewer)(?=[\s\S]*Audit read-only reviewer)(?=[\s\S]*Phap Che legal reviewer)(?=[\s\S]*Out-of-scope negative account)(?=[\s\S]*P6-04, P2-18 and P5-03 evidence exists)/i,
...
requireText(
  "components/ttgdtx/ttgdtx-production-execution-queue.tsx",
  /(?=[\s\S]*PRODUCTION_UAT_LAUNCH_STEPS)(?=[\s\S]*data-ttgdtx-uat-launch-plan="P2-18_P5-03")(?=[\s\S]*First UAT launch plan: P2-18 \+ P5-03)(?=[\s\S]*signed browser UAT)(?=[\s\S]*Use synthetic accounts plus P6-04 real-accounting\s+queue\/result proof)(?=[\s\S]*store proof outside Git\/Codex\/chat)(?=[\s\S]*signed evidence required)(?=[\s\S]*Open UAT route)/i,
