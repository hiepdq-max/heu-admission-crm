...
requireText(
  inventory,
  /npm\.cmd run check:heu-fast-local-loop[\s\S]*PASS_LOCAL fast loop[\s\S]*IT\/Data daily control[\s\S]*current-state inventory[\s\S]*Vietnamese text encoding[\s\S]*HEU_FAST_LOOP_WORKTREE[\s\S]*HEU_FAST_LOOP_WORKTREE_AREAS[\s\S]*HEU_FAST_LOOP_AREA_SAMPLE[\s\S]*HEU_FAST_LOOP_NEXT_GUARDS[\s\S]*HEU_FAST_LOOP_WORKTREE_SAMPLE[\s\S]*HEU_FAST_LOOP_WORKTREE_SCOPE[\s\S]*app[\s\S]*components[\s\S]*docs[\s\S]*scripts[\s\S]*database[\s\S]*other[\s\S]*up to three changed paths per area[\s\S]*--runtime[\s\S]*node --check[\s\S]*npx\.cmd eslint[\s\S]*migration-order[\s\S]*SQL object map[\s\S]*HEU_FAST_LOOP_RUNTIME_PREFLIGHT[\s\S]*active Next dev\/build process[\s\S]*\.next\/lock[\s\S]*default mode skips lint\/build and user-account security[\s\S]*DIRTY_WARN_ONLY[\s\S]*preserves existing changes[\s\S]*no account creation, password handling, email, task, migration, UAT, evidence acceptance, finance reliance, owner GO or production GO[\s\S]*-- --security[\s\S]*-- --strict-worktree/i,
  "IT/Data fast local control loop evidence",
);
requireText(
  inventory,
