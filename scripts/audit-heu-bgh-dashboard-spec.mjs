...
const realOps08FinalOwnerIntake = read(
  "docs/HEU_REAL_OPS_08_FINAL_OWNER_GONOGO_INTAKE_20260702.md",
);
...
requireText(
  realOps08FinalOwnerIntake,
  /(?=[\s\S]*Status:\s*PASS_LOCAL_FINAL_OWNER_INTAKE)(?=[\s\S]*REAL_OPS_08_FINAL_OWNER_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*REAL-OPS-08-OWNER-01)(?=[\s\S]*REAL-OPS-08-OWNER-06)(?=[\s\S]*REAL-OPS-01 through REAL-OPS-07)(?=[\s\S]*P0-10 redaction acceptance)(?=[\s\S]*P0-14 evidence binder)(?=[\s\S]*UAT-ROUTE-01 through UAT-ROUTE-11)(?=[\s\S]*P5_03_CONTROLLED_TRIAL_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*FIN_START_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*FIN_DAY1_RESULT_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*ACCESS_RETAIN \/ REVOKE_OR_REDUCE \/ BLOCKED)(?=[\s\S]*P0_09_ACCEPT \/ NO_GO \/ BLOCKED)(?=[\s\S]*P0_09_FINAL_GO \/ NO_GO \/ BLOCKED)(?=[\s\S]*data-heu-real-ops-08-final-owner-intake="REAL-OPS-08_FINAL_OWNER")(?=[\s\S]*does not approve production, backup, restore, migration,\s+legal waiver, finance action, UAT acceptance, evidence acceptance, payout,\s+dashboard reliance, owner GO\/NO-GO or production GO)(?=[\s\S]*does not mean production, backup, restore, migration,\s+legal waiver, finance action, UAT, evidence, payout, dashboard reliance, owner\s+GO\/NO-GO or production operation is approved)/i,
  "REAL-OPS-08 final owner GO/NO-GO intake source document",
  "docs/HEU_REAL_OPS_08_FINAL_OWNER_GONOGO_INTAKE_20260702.md",
);
requireText(
