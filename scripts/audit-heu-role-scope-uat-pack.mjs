...
requireText(pack, /Do not paste passwords, temporary passwords, OTPs, reset links, account\s+activation\/invite links, API keys, service-role keys, CCCD, private phone\s+numbers, bank accounts, bank statements, vouchers or raw student identity data/i, "secret and sensitive-data boundary");
...
if (!/Do not test with real passwords, temporary passwords, account activation\/invite\s+links, OTPs, service keys or bank credentials/i.test(runbook)) {
  fail("TTGDTX role-scope runbook must keep the no-secret rule.");
}
if (!/send real passwords, temporary passwords or account activation\/invite links into\s+Codex\/chat/i.test(runbook)) {
