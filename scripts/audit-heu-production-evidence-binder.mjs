...
requireText(
  source,
  /(?=[\s\S]*CONTROLLED_SENSITIVE)(?=[\s\S]*CONTROLLED_REDACTED)(?=[\s\S]*service-role key)(?=[\s\S]*password)(?=[\s\S]*temporary password)(?=[\s\S]*OTP)(?=[\s\S]*password reset link)(?=[\s\S]*account activation\/invite link)(?=[\s\S]*raw student PII)(?=[\s\S]*bank statements?)(?=[\s\S]*AI(?:-produced)? approvals?)/i,
...
requireText(
  component,
  /(?=[\s\S]*data-ttgdtx-production-evidence-binder="P0-14")(?=[\s\S]*P0-14 production evidence binder)(?=[\s\S]*PASS_LOCAL only)(?=[\s\S]*PRODUCTION_EVIDENCE_REQUIREMENTS)(?=[\s\S]*PRODUCTION_GOVERNANCE_ASSURANCE_STEPS)(?=[\s\S]*PRODUCTION_UAT_LAUNCH_STEPS)(?=[\s\S]*governanceEvidenceCases)(?=[\s\S]*financeRelianceEvidenceCases)(?=[\s\S]*NO-GO until signed)(?=[\s\S]*what proof is needed, where\s+it must live, who signs it and what must never be pasted into\s+Git\/Codex\/chat)(?=[\s\S]*Forbidden:[\s\S]*item\.forbiddenContent)(?=[\s\S]*data-p014-controlled-evidence-intake-ledger="P0-14")(?=[\s\S]*P0-14 controlled evidence intake ledger)(?=[\s\S]*P0_14_INTAKE_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*evidence ID, controlled folder reference,\s+evidence class, redaction reviewer, owner signature state and\s+blocker decision)(?=[\s\S]*data-p014-governance-evidence-checkpoint="P6-04_P6-03")(?=[\s\S]*P0-14 governance evidence checkpoint: P6-04 \+ P6-03)(?=[\s\S]*controlled references, redaction checks and human sign-off)(?=[\s\S]*role leak, missing trace row, broad\s+access path or unsigned evidence keeps P0-14 NO-GO)(?=[\s\S]*P6_04_SCOPE \+ P6_03_TRACE)(?=[\s\S]*data-p014-finance-reliance-evidence-checkpoint="P2-18_P5-03_P6-04")(?=[\s\S]*P0-14 finance reliance evidence checkpoint: P2-18 \+ P5-03 \+ P6-04)(?=[\s\S]*Dashboard and Finance Desk evidence must cite the P6-04 real\s+accounting user queue\/result proof and P0-17 real-user access\s+closure decision)(?=[\s\S]*open access closure)(?=[\s\S]*P2_18_P5_03_FINANCE_PROOF \/ NO_GO \/ BLOCKED)(?=[\s\S]*data-heu-real-accounting-user-uat-queue)(?=[\s\S]*data-heu-real-accounting-user-result-template)(?=[\s\S]*data-p014-real-user-access-closure-proof="P0-17-P6-04")(?=[\s\S]*ACCESS_RETAIN \/ REVOKE_OR_REDUCE \/ BLOCKED)(?=[\s\S]*data-heu-real-user-access-closure)(?=[\s\S]*step\.code === "P5-03")(?=[\s\S]*data-p014-finance-controlled-trial-evidence="P5-03-TRIAL-EVID")(?=[\s\S]*P5-03-TRIAL-EVID-001 through P5-03-TRIAL-EVID-005)(?=[\s\S]*P5_03_CONTROLLED_TRIAL_READY \/ NO_GO \/ BLOCKED)(?=[\s\S]*data-p014-production-evidence-closure-tracker="P0-14")(?=[\s\S]*P0-14 production evidence closure tracker)(?=[\s\S]*P0_14_CLOSE \/ NO_GO \/ BLOCKED)(?=[\s\S]*controlled evidence reference exists)(?=[\s\S]*Missing proof keeps production NO-GO)(?=[\s\S]*Stop if proof is missing)(?=[\s\S]*Forbidden content stays out of Git\/Codex\/chat)(?=[\s\S]*service-role keys,\s+passwords, temporary passwords, OTPs, password reset links,\s+account\s+activation\/invite links)(?=[\s\S]*raw student PII)(?=[\s\S]*bank statements)(?=[\s\S]*AI-produced approvals)/i,
