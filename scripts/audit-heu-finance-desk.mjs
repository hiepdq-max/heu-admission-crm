...
requireText(
  read("docs/HEU_IMPLEMENTATION_LOG.md"),
  /## 2026-07-02 - Finance Desk Safe Pilot Order Documentation[\s\S]*finance-official-operation-gate\.tsx[\s\S]*HEU_FINANCE_DESK_UAT_RUNBOOK_20260627\.md[\s\S]*HEU_FINANCE_DESK_CONTROLLED_TRIAL_PLAN_20260630\.md[\s\S]*FIN-PILOT-01[\s\S]*FIN-PILOT-05[\s\S]*FIN_PILOT_READY \/ NO_GO \/ BLOCKED[\s\S]*HEU_SYSTEM_BUILD_BACKLOG\.md[\s\S]*TTGDTX_9PLUS_PILOT_PRODUCTION_CHECKLIST\.md[\s\S]*HEU_CURRENT_STATE_INVENTORY\.md[\s\S]*HEU_MODULE_READINESS_GAP_MATRIX_20260628_V01_DRAFT\.md[\s\S]*secure account creation outside Codex\/chat[\s\S]*narrow TTGDTX scope[\s\S]*P6-04 negative-account proof[\s\S]*P2-18\/P5-03 read-only[\s\S]*result ledger and access closure before expansion[\s\S]*does not create accounts[\s\S]*send invites[\s\S]*store passwords[\s\S]*grant access[\s\S]*execute UAT[\s\S]*accept evidence[\s\S]*approve finance reliance[\s\S]*approve access closure[\s\S]*move money[\s\S]*issue bank\s+instructions[\s\S]*mark production GO/i,
