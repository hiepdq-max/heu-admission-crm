...
| P0-08 | Expose TTGDTX production readiness guard in app | IT/Data + Process owners | PASS_LOCAL | `components/ttgdtx/ttgdtx-production-readiness-guard.tsx`; TTGDTX landing guard renders shared `PRODUCTION_BLOCKERS` from `lib/production-readiness.ts`; `components/ttgdtx/ttgdtx-uat-signoff-guard.tsx`; governance UAT execution readiness for P6-04/P6-03; UAT run closure tracker; UAT execution closure template; UAT operator handoff; `components/ttgdtx/ttgdtx-production-execution-queue.tsx`; safe iteration loop for one blocker, one audit, controlled proof and green-guard commit; P0-03/Step90-Step110 infra readiness plan for backup/restore proof and migration-order sign-off; P0-19/P3-01/P3-02 gate-handover readiness plan for legal/finance gate and handover UAT before finance reliance; P6-04/P6-03 governance assurance plan for role/workspace scope and audit-log traceability; P2-18/P5-03 UAT launch plan for first signed browser UAT tracks; P6-06/P2-17 risk closure plan for hard-delete/cascade and payout duplicate blockers; `npm.cmd run audit:ttgdtx-production-readiness-guard`; production remains NO-GO until backup, signed UAT and owner approval |
| P0-09 | Owner Go/No-Go sign-off pack | BGH + IT/Data + KHTC + PHAP_CHE + Audit | PASS_LOCAL | `docs/TTGDTX_PRODUCTION_OWNER_SIGNOFF_PACK_20260627.md`; `docs/TTGDTX_UAT_OPERATOR_HANDOFF_20260627.md`; `docs/HEU_LEAD_LIFECYCLE_HANDOVER_UAT_RUNBOOK_20260628.md`; signed P3-01/P3-02 lifecycle and handover UAT; `docs/HEU_NON_TTGDTX_CASCADE_FINDING_REGISTER_20260628.md`; `components/ttgdtx/ttgdtx-owner-go-no-go-evidence-checklist.tsx`; owner GO/NO-GO acceptance matrix; owner GO/NO-GO decision manifest; `npm.cmd run audit:ttgdtx-production-owner-signoff-pack`; owner GO/NO-GO still required |
...
| P0-17 | User account temporary password security | IT/Data + ADMIN | PASS_LOCAL | `components/settings/user-create-form.tsx`; `app/settings/actions.ts`; `scripts/audit-heu-user-account-security.mjs`; create-user flow blocks weak/reused temporary password signals, warns not to send passwords through Codex/chat/email notes/attachments and does not display keys or log temporary passwords |
| P0-18 | Data Master / Report View compatibility bridge | BGH + IT/Data + KHTC + Audit + process owners | PASS_LOCAL | `docs/HEU_DATA_MASTER_REPORT_VIEW_COMPATIBILITY_20260628_V01_DRAFT.md`; `components/reports/data-master-report-view-bridge-panel.tsx`; `/reports`; non-destructive `STUDENT_MASTER`/`CLASS_MASTER`/`COHORT_MASTER` compatibility design; DQ-DM-01 through DQ-DM-04 checkpoints; DESIGN_ONLY and DRAFT_CONTROL; does not create production SQL, merge source data, import real data, approve report-view signoff, approve dashboard reliance or mark production GO |
