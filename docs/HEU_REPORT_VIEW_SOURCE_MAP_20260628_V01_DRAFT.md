...
| RV_SHORT_COURSE_ATTENDANCE_PAYMENT | `short_course_data_foundation_summary`; `short_course_dashboard_kpis`; `short_course_exception_summary`; `short_course_exception_register`; `short_attendance_session_readiness`; `short_bhxh_policy_case_readiness`; `short_finance_invoice_readiness`; `short_payment_readiness`; `docs/HEU_SHORT_COURSE_REPORT_VIEW_SOURCE_RECONCILIATION_CHECKLIST_20260703.md` | `/short-course`; Short Course dashboard | Class/student/attendance/payment linkage; exception register; attendance lock evidence; SC-RV-EVID-01 through SC-RV-EVID-06 source reconciliation | DAO_TAO + KHTC + IT_DATA + Audit | SOURCE_MAP_DRAFT |
...
| DQ-RV-06 | Short Course | Class, student, attendance, invoice and payment linkage exists; SC-RV-EVID-01 through SC-RV-EVID-06 report-view source reconciliation is prepared | Payment period is closed without attendance/audit |
...
| RV-EVID-05 | RV_SHORT_COURSE_ATTENDANCE_PAYMENT | Short Course attendance/payment UAT; BHXH policy proof; report-view signoff; `docs/HEU_SHORT_COURSE_REPORT_VIEW_SOURCE_RECONCILIATION_CHECKLIST_20260703.md`; SC-RV-EVID-01 through SC-RV-EVID-06 | SC_ATTENDANCE_PAYMENT_READY / NO_GO / BLOCKED; SC_REPORT_VIEW_SOURCE_RECONCILIATION_READY / NO_GO / BLOCKED | Payment period or dashboard is relied on before attendance lock, finance evidence and report-view source reconciliation are signed |
