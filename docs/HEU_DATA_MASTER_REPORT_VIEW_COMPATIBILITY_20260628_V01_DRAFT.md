...
| CV_FACULTY_DEPARTMENT_COMPAT | FACULTY_DEPARTMENT_MASTER | `admission_departments`, staff/position controls and future owner-signed Khoa register | RV_KHOA_GIANG_VIEN_DELIVERY | KHOA-DQ-01 faculty/bo mon owner map and program scope | DESIGN_ONLY |
| CV_TEACHER_PROFILE_COMPAT | TEACHER_PROFILE_MASTER | `users_profile`, role/position controls and future HR/PHAP_CHE teacher display-field register | RV_KHOA_GIANG_VIEN_DELIVERY | KHOA-DQ-02 privacy class, allowed fields and contract/evidence route | DESIGN_ONLY |
...
| RV_KHOA_GIANG_VIEN_DELIVERY | FACULTY_DEPARTMENT_MASTER, TEACHER_PROFILE_MASTER, CLASS_MASTER, TEACHING_DELIVERY_MASTER, REPORT_VIEW_MASTER_CONTRACT | Use Khoa delivery compatibility only for UAT preparation, source reconciliation and read-only review through `docs/HEU_KHOA_GIANG_VIEN_EVIDENCE_TRACE_SOURCE_RECONCILIATION_CHECKLIST_20260703.md`, KHOA_EVIDENCE_TRACE_READY / NO_GO / BLOCKED and KHOA_SOURCE_RECONCILIATION_READY / NO_GO / BLOCKED | Khoa owner signoff, teacher profile privacy approval, source reconciliation, signed UAT and report-view owner signoff are missing |
