...
| HEU Finance Desk read-only cockpit | KHTC + BGH + IT_DATA | PASS_LOCAL | `app/finance-desk/page.tsx`; `database/step111_heu_finance_desk.sql`; `docs/modules/HEU_FINANCE_DESK_MVP_SPEC_20260627.md`; `npm.cmd run audit:heu-finance-desk`; permission/workspace-scope gate, read-only TTGDTX dashboard/import/source-control views and shared VND formatter; does not approve finance action, production migration, UAT acceptance or owner GO | YES | KHTC/BGH use a combined cockpit before role scope, source facts and signed UAT are proven |
