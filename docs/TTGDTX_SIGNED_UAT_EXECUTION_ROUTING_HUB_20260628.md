...
| UAT-ROUTE-11 | P0-09 | `/ttgdtx` | `docs/TTGDTX_PRODUCTION_OWNER_SIGNOFF_PACK_20260627.md` | BGH + IT_DATA + KHTC + PHAP_CHE + AUDIT + TRUONG_PHONG | Final owner decision manifest with signed UAT, evidence binder, migration, backup, role, Finance Day-1 result ledger, P0-17 access closure decision, audit and risk-closure references | SIGNED_UAT_READY / NO_GO / BLOCKED | Any required owner signs NO-GO/BLOCKED, Finance Day-1 result ledger is missing, P0-17 access closure is missing, any proof path is uncontrolled, or any prerequisite UAT remains pending | `npm.cmd run audit:ttgdtx-production-owner-signoff-pack` |
