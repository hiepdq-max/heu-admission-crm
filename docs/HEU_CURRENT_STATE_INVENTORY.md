...
| `npm.cmd run check:heu-fast-local-loop` | PASS_LOCAL fast loop for IT/Data daily control, current-state inventory and Vietnamese text encoding; prints `HEU_FAST_LOOP_WORKTREE`, `HEU_FAST_LOOP_WORKTREE_AREAS`, `HEU_FAST_LOOP_AREA_SAMPLE`, `HEU_FAST_LOOP_NEXT_GUARDS`, `HEU_FAST_LOOP_WORKTREE_SAMPLE` and `HEU_FAST_LOOP_WORKTREE_SCOPE` before guards; area counts group dirty paths into `app`, `components`, `docs`, `scripts`, `database` and `other`, while area samples show up to three changed paths per area; next-guard hints route app/components changes to `--runtime`, docs changes to current-state, implementation-log and Vietnamese text audits, script changes to `node --check` plus `npx.cmd eslint`, database changes to migration-order and SQL object map audits, and handoff checks to `--strict-worktree`; runtime mode prints `HEU_FAST_LOOP_RUNTIME_PREFLIGHT` and returns `NO_GO` before lint/build when an active Next dev/build process or `.next/lock` would make build verification unreliable; default mode skips lint/build and user-account security, treats dirty worktree as `DIRTY_WARN_ONLY`, preserves existing changes and stays read-only with no account creation, password handling, email, task, migration, UAT, evidence acceptance, finance reliance, owner GO or production GO; use `-- --security` for P0-17/P6-04 slices and `-- --strict-worktree` only for clean handoff checks |
...
| Process discovery/navigation | Shared TTGDTX process labels, Search suggestions, `/search` quick-open top 3 results with `data-heu-search-quick-open="P1-11_SEARCH_QUICK_OPEN"` and no-overflow guard, documents quick access with `data-heu-documents-quick-open="P0-14_DOCUMENTS_QUICK_OPEN_TOP6"` and no-overflow guard, and `/ttgdtx` quick finder show business name before P2 code | PASS_LOCAL; signed browser UAT pending |
...
| Short Course attendance/payment gap pack | `/short-course` shows the Short Course Attendance/Payment Gap Pack from `docs/HEU_SHORT_COURSE_ATTENDANCE_PAYMENT_GAP_PACK_20260628_V01_DRAFT.md`, owner signoff manifest `docs/HEU_SHORT_COURSE_OWNER_SIGNOFF_MANIFEST_20260702.md` and UAT result ledger `docs/HEU_SHORT_COURSE_UAT_RESULT_LEDGER_TEMPLATE_20260703.md`, including SC-AP-01 through SC-AP-08, SC-REV-01 through SC-REV-06, SC-SIGN-01 through SC-SIGN-06, SC-UAT-LEDGER-01 through SC-UAT-LEDGER-08, `SC_ATTENDANCE_PAYMENT_READY / NO_GO / BLOCKED`, `SC_REVIEW_READY / NO_GO / BLOCKED`, `SHORT_COURSE_OWNER_READY / NO_GO / BLOCKED`, `SC_UAT_RESULT_READY / NO_GO / BLOCKED`, read-only quick access with `data-heu-short-course-quick-access="P9-01_SHORT_COURSE_QUICK_ACCESS"` and `data-heu-short-course-quick-access-overflow-guard="P9-01_SHORT_COURSE_QUICK_ACCESS_NO_OVERFLOW"`, and attendance/BHXH/meal/finance stop conditions | PASS_LOCAL; no attendance lock, BHXH decision, meal/allowance payment, HR payment, invoice/payment verification, period close, statutory accounting, UAT acceptance, evidence acceptance, owner GO/NO-GO or production GO approved |
