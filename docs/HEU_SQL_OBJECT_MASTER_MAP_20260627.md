...
| `DATA_CONFIRMATION_TASK_MASTER` | `heu_data_confirmation_tasks`, `heu_data_confirmation_task_status_history`, `heu_data_confirmation_task_center`, `heu_data_confirmation_task_status_timeline`, `route_data_confirmation_task`, `confirm_data_confirmation_task`, `due_date_or_batch`, `owner_decision_ref`, `assigned_user_id`, `owner_user_id`, `ASSIGNEE_OR_OWNER_REQUIRED`, `CONTROLLED_PILOT_DEPARTMENT_ONLY`, `CONFIRM_SUBMITTER_SCOPE_LOCK`, `CONFIRM_FROM_CHO_XAC_NHAN_ONLY` | Step121 Data Confirmation Task Center schema contract exists as PASS_LOCAL_SCHEMA_CONTRACT for routing approved metadata to `CHO_XAC_NHAN` and department/user confirmation tasks with required due/batch, owner-decision references and an assigned/owner lane; status timeline scope parity uses assigned/owner user IDs; `CONTROLLED_PILOT_DEPARTMENT_ONLY` keeps task ownership routing to the six controlled pilot departments; `CONFIRM_SUBMITTER_SCOPE_LOCK` exposes `can_current_user_confirm` so only an allowed submitter can confirm a visible task; `CONFIRM_FROM_CHO_XAC_NHAN_ONLY` keeps confirmation limited to `CHO_XAC_NHAN`; DA_KHOA lock requires note and controlled evidence ref | Migration candidate only; must not auto-seed real tasks and must not seed real tasks automatically, import raw data or rely on the task center until backup/restore proof, signed migration order, signed UAT, controlled evidence refs and owner GO/NO-GO are completed outside Git/Codex/chat |
...
| Step121 DCTC schema contract | `heu_data_confirmation_tasks`, `heu_data_confirmation_task_status_history`, `heu_data_confirmation_task_center`, `heu_data_confirmation_task_status_timeline`, `route_data_confirmation_task`, `confirm_data_confirmation_task`, `due_date_or_batch`, `owner_decision_ref`, `assigned_user_id`, `owner_user_id`, `ASSIGNEE_OR_OWNER_REQUIRED`, `CONTROLLED_PILOT_DEPARTMENT_ONLY`, `CONFIRM_SUBMITTER_SCOPE_LOCK`, `CONFIRM_FROM_CHO_XAC_NHAN_ONLY`, `DA_KHOA lock requires note and controlled evidence ref` | `DATA_CONFIRMATION_TASK_MASTER`, `WORKFLOW_REQUEST_MASTER`, `AUDIT_LOG`, `REPORT_VIEW_MASTER_CONTRACT` |
