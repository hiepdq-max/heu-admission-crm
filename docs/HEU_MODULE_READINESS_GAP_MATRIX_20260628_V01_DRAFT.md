...
| Report View Register | Draft register lists TTGDTX, HOU, Short Course, Audit and AI views; source map draft exists | CAN_SUA | Report View Register UI, source map, KPI dictionary shell | Dashboard reading raw workbooks/tables or restricted source data | Data Quality Check Log and owner signoff per report view |
...
| Cong no dashboard from approved view | CAN_SUA | Report View Register exists but signoff is still draft |
...
| 2 | Report View Register hardening | CAN_SUA | Source map draft and KPI dictionary shell created; next build is read-only UI/status guard |
