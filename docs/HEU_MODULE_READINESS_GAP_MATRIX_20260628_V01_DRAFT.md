...
| Finance Desk | `/finance-desk`, Step111, MVP spec, UAT runbook, controlled trial plan, Finance safe pilot order and reliance decision manifest exist | DAT for read-only cockpit | Read-only filters, evidence checklist, report-view source map, UAT support and `FIN_PILOT_READY / NO_GO / BLOCKED` order before expansion | Statutory accounting, voucher posting, bank transfer, payment approval, broad pilot expansion | Signed P5-03 browser UAT, access closure and reliance decision |
