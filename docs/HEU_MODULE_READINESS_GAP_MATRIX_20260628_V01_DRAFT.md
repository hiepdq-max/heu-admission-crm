...
| Finance Desk | `/finance-desk`, Step111, MVP spec, UAT runbook, controlled trial plan, Finance safe pilot order, Day-1 accountant handoff and reliance decision manifest exist | DAT for read-only cockpit | Read-only filters, evidence checklist, report-view source map, UAT support, `FIN_PILOT_READY / NO_GO / BLOCKED` order and `FIN_ACCOUNTANT_HANDOFF_READY / NO_GO / BLOCKED` operator boundary before expansion | Statutory accounting, voucher posting, bank transfer, payment approval, broad pilot expansion, accountant bypass of escalation route | Signed P5-03 browser UAT, access closure and reliance decision |
